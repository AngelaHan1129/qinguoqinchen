...
function createRagService() {
  return {
    ...
    userDocuments: [],
    userChunks: [],
    legalDocuments: [],  // 新增法律文件
    legalChunks: [],     // 新增法律文件塊

    // 新增：攝取法律文件
    async ingestLegalDocument(data) {
      console.log('📚 攝取法律文件到資料庫...');

      try {
        const docId = `legal_doc_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
        ...
        const legalDocument = {
          ...
          source: data.source || '全國法規資料庫',
          documentType: data.documentType || '法律',
          jurisdiction: data.jurisdiction || '中華民國',
          lawCategory: data.lawCategory || '一般法',
        ...
        this.legalDocuments.push(legalDocument);

        // 特殊的法律文件分塊處理
        const chunks = this.chunkLegalDocument(data.content, docId, data);
        this.legalChunks.push(...chunks);

        // 為每個塊生成向量嵌入
        for (const chunk of chunks) {
          chunk.embedding = this.generateMockEmbedding(chunk.content);
          chunk.legalConcepts = this.extractLegalConcepts(chunk.content);
          chunk.keywords = this.extractLegalKeywords(chunk.content);
        }

        // 儲存到 PostgreSQL（實際實作）
        await this.saveLegalToPostgreSQL(legalDocument, chunks);

        console.log(`✅ 法律文件已儲存: ${docId}, 共 ${chunks.length} 個條文塊`);

        return {
          success: true,
          documentId: docId,
          chunksCount: chunks.length,
          lawCategory: data.lawCategory,
          message: `法律文件「${data.title}」已成功儲存並分割為 ${chunks.length} 個條文片段`
        };
      } catch (error) {
        console.error('❌ 攝取法律文件失敗:', error.message);
        throw error;
      }
    },

    // 新增：查詢法律資料
    async queryLegalDocuments({ question, lawCategory, source, articleNumber, topK = 5, minSimilarity = 0.7 }) {
      console.log('⚖️ 查詢法律資料...', { question, lawCategory, source });

      try {
        // 過濾法律文件塊
        let filteredChunks = [...this.legalChunks];

        if (lawCategory) {
          filteredChunks = filteredChunks.filter(chunk =>
            this.legalDocuments.find(doc =>
              doc.id === chunk.documentId && doc.lawCategory === lawCategory
            )
          );
        }
        ...
        if (source) {
          filteredChunks = filteredChunks.filter(chunk =>
            this.legalDocuments.find(doc =>
              doc.id === chunk.documentId && doc.source === source
            )
          );
        }
        ...
        if (articleNumber) {
          filteredChunks = filteredChunks.filter(chunk =>
            chunk.articleReference && chunk.articleReference.includes(articleNumber)
          );
        }

        // 計算相似度並排序
        const relevantChunks = this.searchLegalChunks(question, { minSimilarity }, topK, filteredChunks);

        return {
          relevantChunks,
          sources: relevantChunks.map(chunk => {
            const doc = this.legalDocuments.find(d => d.id === chunk.documentId);
            return {
              documentId: chunk.documentId,
              chunkId: chunk.id,
              title: doc?.title || '未知法規',
              lawCategory: doc?.lawCategory || '一般法',
              articleReference: chunk.articleReference,
              similarity: chunk.similarity,
              legalConcepts: chunk.legalConcepts || [],
              preview: chunk.content.substring(0, 200) + '...'
            };
          }),
          totalFound: relevantChunks.length,
          searchParams: { lawCategory, source, articleNumber, topK, minSimilarity }
        };
      } catch (error) {
        console.error('❌ 查詢法律資料失敗:', error.message);
        throw error;
      }
    },

    // 新增：法律專用分塊處理
    chunkLegalDocument(content, docId, metadata, maxChunkSize = 600) {
      const chunks = [];

      // 根據條文結構分割（例如：第X條、第X項等）
      const articles = content.split(/(第\d+條|第\d+項|第\d+款)/);
      let currentArticle = '';
      let chunkIndex = 0;

      for (let i = 0; i < articles.length; i += 2) {
        const articleHeader = articles[i - 1] || '';
        const articleContent = articles[i] || '';

        if (articleContent.trim().length > 0) {
          const fullContent = (articleHeader + articleContent).trim();
          ...
          if (fullContent.length > maxChunkSize) {
            ...
            for (const sentence of sentences) {
              if (currentChunk.length + sentence.length > maxChunkSize && currentChunk.length > 0) {
                chunks.push(this.createLegalChunk(currentChunk, docId, chunkIndex, articleHeader, metadata));
            ...
            chunks.push(this.createLegalChunk(fullContent, docId, chunkIndex, articleHeader, metadata));
            chunkIndex++;
          }
        }
      }
    ...
    // 新增：建立法律塊
    createLegalChunk(content, docId, chunkIndex, articleRef, metadata) {
      return {
        id: `${docId}_legal_chunk_${chunkIndex}`,
        documentId: docId,
        chunkIndex: chunkIndex,
        content: content.trim(),
        articleReference: articleRef || null,
        chunkType: this.determineLegalChunkType(content),
        lawCategory: metadata.lawCategory || '一般法',
        createdAt: new Date().toISOString()
      };
    },
    ...
    extractLegalConcepts(content) {
      const concepts = [];
      const legalTerms = [
        '個人資料', '隱私權', '資料處理', '同意', '告知義務',
        '電子簽章', '數位憑證', '身分驗證', 'eKYC', 'KYC',
        '生物特徵', '人臉辨識', '指紋', '聲紋', 'Deepfake',
        '資料安全', '加密', '存取控制', '稽核軌跡'
      ];
    ...
    // 新增：判斷法律塊類型
    determineLegalChunkType(content) {
      if (content.includes('第') && content.includes('條')) return '條文';
      if (content.includes('說明') || content.includes('立法理由')) return '說明';
      if (content.includes('案例') || content.includes('實務見解')) return '案例';
      if (content.includes('罰') || content.includes('處')) return '罰則';
      return '一般條文';
    },

    // 新增：法律專用搜尋
    searchLegalChunks(question, filters, topK = 5, chunks = null) {
      const targetChunks = chunks || this.legalChunks;
      const questionLower = question.toLowerCase();

      // 法律專用關鍵詞權重
      const legalKeywords = {
        '個人資料': 0.3, '隱私': 0.3, 'ekyc': 0.4, 'kyc': 0.4,
        'deepfake': 0.5, '人臉': 0.3, '生物特徵': 0.3, '身分驗證': 0.3,
        '電子簽章': 0.3, '數位憑證': 0.3, '資料處理': 0.2, '同意': 0.2
      };
      ...
      const scoredChunks = targetChunks.map(chunk => {
        ...
        // 基本文字匹配
        Object.entries(legalKeywords).forEach(([keyword, weight]) => {
        ...
        // 法律概念匹配
        if (chunk.legalConcepts) {
          chunk.legalConcepts.forEach(concept => {
        ...
        // 條文引用匹配
        if (chunk.articleReference && questionLower.includes('條')) {
          score += 0.2;
        }
      ...
      return scoredChunks
        .filter(chunk => chunk.similarity >= (filters.minSimilarity || 0.5))
        .sort((a, b) => b.similarity - a.similarity)
        .slice(0, topK);
    },

    // 新增：儲存法律資料到 PostgreSQL
    async saveLegalToPostgreSQL(document, chunks) {
      console.log('💾 儲存法律資料到 PostgreSQL...', {
        documentId: document.id,
        title: document.title,
        chunksCount: chunks.length,
        lawCategory: document.lawCategory
      });

      // 實際實作時，這裡會連接到真實的 PostgreSQL 資料庫
      // 使用你現有的資料庫連接邏輯
    },

    // 擴展統計方法
    getLegalStats() {
      const legalDocs = this.legalDocuments;
      const legalChunks = this.legalChunks;

      return {
        totalLegalDocuments: legalDocs.length,
        totalLegalChunks: legalChunks.length,
        lawCategories: [...new Set(legalDocs.map(doc => doc.lawCategory))],
        documentTypes: [...new Set(legalDocs.map(doc => doc.documentType))],
        sources: [...new Set(legalDocs.map(doc => doc.source))],
        chunkTypes: [...new Set(legalChunks.map(chunk => chunk.chunkType).filter(Boolean))],
        averageLegalChunkSize: legalChunks.length > 0 ?
          Math.round(legalChunks.reduce((sum, chunk) => sum + chunk.content.length, 0) / legalChunks.length) : 0,
        recentLegalDocuments: legalDocs
...
*此為基於現有法規資料庫的初步分析，建議進一步諮詢法律專家。*`,
  ...
  // 法律資料管理路由
  console.log('⚖️ 註冊法律資料管理路由...');

  // 攝取法律文件
  app.post('/legal/ingest', async (req, res) => {
    console.log('📚 攝取法律文件', 'Body:', req.body);
    try {
      const { title, content, source, documentType, jurisdiction, lawCategory, articleNumber, metadata } = req.body;

      if (!content || content.trim().length === 0) {
        return res.status(400).json({
          success: false,
          error: '請提供法律條文內容 (content 參數)'
        });
      }

      const result = await ragService.ingestLegalDocument({
        title: title || '未命名法規',
        content,
        source: source || '全國法規資料庫',
        documentType: documentType || '法律',
        jurisdiction: jurisdiction || '中華民國',
        lawCategory: lawCategory || '一般法',
      ...
      res.json({
        success: true,
        message: '法律文件已成功攝取並建立向量索引',
        data: result
      });
    } catch (error) {
      console.error('❌ 攝取法律文件失敗:', error);
      res.status(500).json({ success: false, error: error.message });
    }
  });
  ...
  // 查詢法律資料
  app.post('/legal/query', async (req, res) => {
    console.log('⚖️ 查詢法律資料', 'Body:', req.body);
    try {
      const { question, lawCategory, source, articleNumber, topK = 5, minSimilarity = 0.7 } = req.body;

      if (!question) {
        return res.status(400).json({
          success: false,
          error: '請提供法律問題 (question 參數)'
        });
      }

      // RAG 查詢法律資料
      const ragResult = await ragService.queryLegalDocuments({
        question,
        lawCategory,
        source,
        articleNumber,
      ...
      // 結合 Gemini AI 生成法律諮詢回答
      const legalContext = ragResult.relevantChunks
        .map(chunk => `【${chunk.articleReference || '條文'}】${chunk.content}`)
        .join('\n\n');

      const legalPrompt = `作為法律專家，請基於以下相關法條回答問題：
...
回答請專業、準確，並明確引用條文依據。`;
      ...
      const aiResponse = await geminiService.generateAttackVector(legalPrompt);

      res.json({
        success: true,
        answer: aiResponse.text,
        legalSources: ragResult.sources,
        searchParams: ragResult.searchParams,
        retrievedChunks: ragResult.totalFound,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      console.error('❌ 法律查詢失敗:', error);
      res.status(500).json({ success: false, error: error.message });
    }
  });

  // 批量匯入法規資料
  app.post('/legal/batch-import', async (req, res) => {
    console.log('📖 批量匯入法規資料');
    try {
      ...
      for (const doc of documents) {
        try {
          const result = await ragService.ingestLegalDocument(doc);
          results.push({ success: true, documentId: result.documentId, title: doc.title });
        } catch (error) {
  ...
  // 獲取法律統計資訊
  app.get('/legal/stats', async (req, res) => {
    console.log('📊 獲取法律資料統計');
    try {
      const stats = ragService.getLegalStats();
      res.json({
        success: true,
        stats,
        timestamp: new Date().toISOString()
      ...
      console.error('❌ 獲取法律統計失敗:', error);
      res.status(500).json({ success: false, error: error.message });
    }
  });

  // 搜尋特定法條
  app.get('/legal/articles/:articleNumber', async (req, res) => {
    console.log('🔍 搜尋特定法條:', req.params.articleNumber);
    try {
      const { articleNumber } = req.params;
      const { lawCategory, source } = req.query;

      const result = await ragService.queryLegalDocuments({
        question: `請提供${articleNumber}的內容`,
        lawCategory,
        source,
        articleNumber,
        topK: 10,
      ...
      res.json({
        success: true,
        articleNumber,
        results: result.relevantChunks,
        totalFound: result.totalFound
      });
    } catch (error) {
      console.error('❌ 搜尋法條失敗:', error);
      res.status(500).json({ success: false, error: error.message });
    }
  });
